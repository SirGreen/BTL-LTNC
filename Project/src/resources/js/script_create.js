...
function createVehicleElement(vehicleInfo) {
  ...
  if (vehicleInfo.Brand === "SUZUKI XL7") {
    ...
    elem.setAttribute(
      "src",
      "floai_ltnc/kisspng-ferrari-458-car-ferrari-f12-2017-ferrari-488-gtb-5af68701a38a217967992315261058576699.png"
    );
    elem.setAttribute("alt", "added_car");
  } else {
  ...
  a2.href = "javascript:void(0)"; // Prevent default link behavior
  a2.onclick = function () {
    // fetch("/car1_admin", {
    //   method: "POST",
    ...
    form.action = "/car1_admin";
    form.method = "POST";
    for (let dataKey in vehicleInfo) {
        const input = document.createElement('input');
        input.name = dataKey;
        input.value = vehicleInfo[dataKey];
